...
async def start(sree, m: Message):
    ...
    if m.chat.id == user.id:
        e = await m.reply((txt.start_01).format(user.mention))
        await rest(2)
        await e.edit_text((txt.start_02).format(me.first_name, owner.username))
        await rest(2)
        await e.edit_text(txt.start_03)
        await rest(1.5)
        await e.delete()
        await m.reply_sticker(txt.start_04)
        await m.reply((txt.start_05).format(user.mention, me.mention))
    if m.chat.id != user.id:
        await m.reply(404)
